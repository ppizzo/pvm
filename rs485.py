...
class AsyncWriteRS485(threading.Thread):
    """Async daily details write thread"""
    def __init__(self, ser, command, delay):
        threading.Thread.__init__(self)
        self.ser = ser
        self.delay = delay
        self.command = command
    def run(self):
        logging.info("RS485 " + self.command[:-1] + " write thread started")
        global go
        while go:
            self.ser.write(self.command)
            time.sleep(self.delay)
        logging.info("RS485 " + self.command[:-1] + " write thread stopped")
...
def start_details_write():
    global details_write_task
    details_write_task = AsyncWriteRS485(ser, DAILY_DETAILS_CMD, mylib.config_details_delay)
    details_write_task.start()

def start_totals_write():
    global totals_write_task
    totals_write_task = AsyncWriteRS485(ser, DAILY_TOTALS_CMD, mylib.config_details_delay * 60)
    totals_write_task.start()
...
def start_read_write():
    ...
    read_write_task = AsyncReadWriteRS485(ser, mylib.config_details_delay)
...
def stop_all():
    ...
    go = False;
    read_task.join()
    details_write_task.join()
    totals_write_task.join()
